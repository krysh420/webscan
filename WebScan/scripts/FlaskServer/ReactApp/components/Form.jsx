...
const Form = () => {
  ...
  return (
    ...
    <div className="form rounded-lg h-[70vh] w-[36%] max-[1050px]:w-full max-[1050px]:h-fit max-sm:p-2 max-[1050px]:py-4 max-[1050px]:px-2 " style={{border:"1px solid #dddddd", boxShadow:" 0 1px 1px 0 rgba(0, 0, 0, 0.2), 0 5px 1px 0 rgba(0, 0, 0, 0.19)"}}>
      ...
      <form className='text-blue-950 flex flex-col min-[1050px]:mt-[16%] min-[1050px]:justify-between min-[1050px]:h-[34vh]'>
        <div className="input flex flex-col px-4 my-2">
        <label htmlFor="port" className={`my-2 ${localStorage.getItem("CurrentMode")==="dark"?"text-blue-50":""}`}>Enter the PORT:</label>
        <input type="text" name='URL' onChange={onchange} value={Data.URL} className={`border border-blue-700 rounded-lg p-2 focus:ring-2 focus:ring-blue-200 focus:outline-none ${localStorage.getItem("CurrentMode")==="dark"?"bg-transparent text-blue-50":""}`}  id='port'/>
        </div>
        ...
        <div className="button flex justify-end mt-4"><button className='bg-blue-600 text-blue-50 p-2 rounded-lg ' onClick={handleSubmit}>Start Scan</button></div>
        </div>
      </form>
    </div>
    </>
